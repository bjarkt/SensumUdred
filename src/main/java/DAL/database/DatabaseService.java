...
public class DatabaseService extends PostgreSqlDatabase implements IDatabaseService {
	...
	public IUser signIn(String username, String password) {
		...
		executeQuery(conn -> {
			PreparedStatement ps1 = conn.prepareStatement("SELECT * FROM users NATURAL JOIN\n" +
					"  (SELECT users_ssn as ssn, password_hash, securitylevel FROM accounts\n" +
					"    JOIN\n" +
					"  haslogin ON accounts.username=?) as t;" +
					"");
	...
	public boolean signUpAccount(String username, String password, int securityLevel) {
		...
		executeQuery(conn -> {
			PreparedStatement ps = conn.prepareStatement("INSERT INTO accounts VALUES (DEFAULT, ?, ?, ?, ?);");
